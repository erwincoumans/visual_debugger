...
#include <wchar.h>
static TinyWin32InternalData2* sData = 0;
...
void TinyWin32Window::pumpMessage() {
  MSG msg;
  // check for messages
...
int getSpecialKeyFromVirtualKeycode(int virtualKeyCode) {
  ...
  switch (virtualKeyCode) {
    case VK_SPACE: {
      keycode = TINY_KEY_SPACE;
      break;
    }
    case VK_RETURN: {
      keycode = TINY_KEY_RETURN;
      break;
    };
    case VK_ESCAPE: {
      keycode = TINY_KEY_ESCAPE;
      break;
    };
    case VK_F1: {
      keycode = TINY_KEY_F1;
    ...
    case VK_NEXT: {
      keycode = TINY_KEY_PAGE_DOWN;
      break;
    }
    case VK_PRIOR: {
      keycode = TINY_KEY_PAGE_UP;
      break;
    }

    case VK_INSERT: {
      keycode = TINY_KEY_INSERT;
      break;
    }
    case VK_BACK: {
      keycode = TINY_KEY_BACKSPACE;
      break;
    }
    case VK_DELETE: {
    ...
    case VK_HOME: {
      keycode = TINY_KEY_HOME;
      break;
    }
    case VK_LEFT: {
      keycode = TINY_KEY_LEFT_ARROW;
      break;
    }
    case VK_UP: {
      keycode = TINY_KEY_UP_ARROW;
      break;
    }
    case VK_RIGHT: {
      keycode = TINY_KEY_RIGHT_ARROW;
      break;
    }
    case VK_DOWN: {
      keycode = TINY_KEY_DOWN_ARROW;
      break;
    }
    case VK_SHIFT: {
      keycode = TINY_KEY_SHIFT;
      break;
    }
    case VK_MENU: {
      keycode = TINY_KEY_ALT;
      break;
    }
    case VK_CONTROL: {
      keycode = TINY_KEY_CONTROL;
      break;
    }
    default: {
...
bool TinyWin32Window::is_modifier_key_pressed(int key) {
...
void TinyWin32Window::set_window_title(const char* titleChar) {
#ifdef _WIN64
...
void TinyWin32Window::create_window(const TinyWindowConstructionInfo& ci) {
  ...
  if (!windowHandle) {
    ...
    //		wcex.hIcon = (HICON)LoadImage(hInstance, "bullet.ico",
...
void TinyWin32Window::switchFullScreen(bool fullscreen, int width, int height,
...
TinyWin32Window::TinyWin32Window() {
  m_data = new TinyWin32InternalData2();
  sData = m_data;
}

TinyWin32Window::~TinyWin32Window() {
  set_keyboard_callback(0);
  set_mouse_move_callback(0);
  set_mouse_button_callback(0);
  set_wheel_callback(0);
  set_resize_callback(0);

  sData = 0;
  delete m_data;
}

void TinyWin32Window::set_render_callback(TinyRenderCallback renderCallback) {}

void TinyWin32Window::close_window() {
  set_keyboard_callback(0);
  set_mouse_move_callback(0);
  set_mouse_button_callback(0);
  set_wheel_callback(0);
  set_resize_callback(0);
  set_render_callback(0);

  DestroyWindow(this->m_data->m_hWnd);
}

void TinyWin32Window::getMouseCoordinates(int& x, int& y) {
  x = m_data->m_mouseXpos;
  y = m_data->m_mouseYpos;
}

void TinyWin32Window::run_main_loop() {}
...
float TinyWin32Window::get_time_in_seconds() { return 0.f; }

void TinyWin32Window::setDebugMessage(int x, int y, const char* message) {}

void TinyWin32Window::set_request_exit() { m_data->m_quit = true; }
bool TinyWin32Window::requested_exit() const { return m_data->m_quit; }

void TinyWin32Window::set_wheel_callback(TinyWheelCallback wheelCallback) {
  m_data->m_wheelCallback = wheelCallback;
}

void TinyWin32Window::set_mouse_move_callback(
    TinyMouseMoveCallback mouseCallback) {
  m_data->m_mouseMoveCallback = mouseCallback;
}

void TinyWin32Window::set_mouse_button_callback(
    TinyMouseButtonCallback mouseCallback) {
  m_data->m_mouseButtonCallback = mouseCallback;
}

void TinyWin32Window::set_resize_callback(TinyResizeCallback resizeCallback) {
...
void TinyWin32Window::set_keyboard_callback(
    TinyKeyboardCallback keyboardCallback) {
  m_data->m_keyboardCallback = keyboardCallback;
}

TinyKeyboardCallback TinyWin32Window::get_keyboard_callback() {
  return m_data->m_keyboardCallback;
}

TinyMouseMoveCallback TinyWin32Window::get_mouse_move_callback() {
  return m_data->m_mouseMoveCallback;
}
TinyMouseButtonCallback TinyWin32Window::get_mouse_button_callback() {
  return m_data->m_mouseButtonCallback;
}
TinyResizeCallback TinyWin32Window::get_resize_callback() {
  return m_data->m_resizeCallback;
}
TinyWheelCallback TinyWin32Window::get_wheel_callback() {
